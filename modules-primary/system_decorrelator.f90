...
! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    num_bin_lyp    = CEILING( ( DBLE(N) / 128.0D0 ) * 200.0D0 )
    !  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    !  A  L  L  O  C  A  T  I  O  N
    !  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ...
  ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    lyp_dec     = ( DLOG( dec ) - DLOG( dec_old ) ) / dt
  ...
  ! INFO - START  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  ! ------------
  ! CALL this to compute the lyapunov from strain and vis
  ! -------------
  ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    DOUBLE PRECISION::str_grow,vis_grow
    ...
    !  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      str_grow = -( ((U_x(i_x,i_y,i_z)-Ub_x(i_x,i_y,i_z))**two)*s_xx(i_x,i_y,i_z)+&
    ...
    !  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      vis_grow = df_x*(-6.0D0*df_x+df_x_px+df_x_py+df_x_pz+df_x_mx+df_x_my+df_x_mz) + &
      ...
      vis_grow = vis * vis_grow / ( l_grd * l_grd )
      ! The viscous diffusion based decorrelation growth

      Lyp_fld( i_x, i_y, i_z ) = ( str_grow + vis_grow ) / Dec_fld( i_x, i_y, i_z )

      dec_dot_str              = dec_dot_str + str_grow
      dec_dot_vis              = dec_dot_vis + vis_grow
    ...
    lyp_avg     = lyp_avg     / N3
    lyp_std     = DSQRT( ( lyp_std / N3 ) - lyp_avg ** two )
  ...
  IMPLICIT NONE
    ...
    LOOP_RZ_902: DO i_z = 0 , N - 1
      ...
      Lyp_fld( i_x, i_y, i_z ) = ( Lyp_fld( i_x, i_y, i_z ) - lyp_avg ) / lyp_std ! Normalized distribution: Mean=0, Std=1
      bin                      = CEILING( ( Lyp_fld( i_x, i_y, i_z ) - lyp_min ) / lyp_bin_size )

      BIN_CHECK_904: IF( (bin .GE. 1 ) .AND. (bin .LE. num_bin_lyp ) ) THEN
        Lyp_pdf( bin )         = Lyp_pdf( bin ) + one
        num_pts_pdf            = num_pts_pdf + 1
      ELSEIF (bin .LT. 1 ) THEN
        Lyp_pdf( 1 )           = Lyp_pdf( 1 ) + one
      ELSEIF (bin .GT. num_bin_lyp ) THEN
    ...
    IF  ( DBLE(num_pts_pdf) / N3 .LT. 0.98D0 ) THEN
      PRINT*," More than 2 percent of values are falling out of the lyapunov histogram bins"
    END IF
  ...
  ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    WRITE(4002,f_d32p17,ADVANCE ='no') lyp_dec
